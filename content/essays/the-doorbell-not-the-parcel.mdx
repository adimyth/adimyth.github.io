...
```mermaid
flowchart LR
  Meta[Meta / WhatsApp] -->|POST webhook| GW[API Gateway<br/>one per tenant]
  GW --> L[Lambda<br/>verify HMAC signature]
  L -->|invalid signature| R[401]
  L -->|valid| Q[(SQS)]
  Q --> P[Poller<br/>on worker box]
  P --> H[Webhook handlers<br/>dedupe, encrypt, persist]
  H --> PG[(Postgres)]
  Q -.->|3 failed attempts| DLQ[(DLQ)]
```
...
```mermaid
sequenceDiagram
  participant W as Poller (worker)
  participant R as Redis pub/sub
  participant A as API service
  participant C as Flutter app
  W->>W: process payload, commit to Postgres
  W->>R: PUBLISH user:{user_id}:events
  R->>A: message_arrived {ids}
  A->>C: SSE event on open stream
  C->>A: GET /conversations/{id}/messages?after_id={last}
  A->>C: new messages, decrypted, scoped to user
```
